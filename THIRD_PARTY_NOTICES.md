...
B&A MCP bundles the components below. Each is available separately for free;
this bundle ships them together as a convenience, disclosed as such on the sales
page. Each remains under its own license.

Same-commit rule: any change to a bundled component (version bump, add, remove)
updates that component's block here in the same commit (WORKFLOW.md rule 4).
