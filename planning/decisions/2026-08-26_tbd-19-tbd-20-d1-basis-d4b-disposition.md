...
- **19.3 — exposure:** add `dirTargets: Set<string>` (the flattened union of `dirTargetsBySrc` values) to `GraphResult`; thread it into `AcceptedLayoutCtx` **in place of** the `routedDirs` field (D1 is that field's only consumer, so it is replaced, not kept alongside). `routedDirs` itself is untouched — it stays computed in `buildGraph` and still feeds `underRoutedDir`, a separate graph-local set, not the `AcceptedLayoutCtx`. Internal only; no output-schema change.
