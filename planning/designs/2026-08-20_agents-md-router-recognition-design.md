...
- **Two wanted catches are preserved — this is why the guard is scoped, not bare.** (1) When routers resolve **≥ 1** edge but cover no significant directory, `coverage` still floors to **0** — the monolithic-route-nowhere repo (e.g. claude-mem `coverage 0 / n50`, one resolved root edge) stays caught. (2) When there are **no routers at all** (`root_absent`, `routing_files === 0`), `coverage` still floors to **0** so a no-context-layer repo with ≥ 1 significant directory yields headline **0** — the worst-case catch in `planning/decisions/2026-08-20_headline-definite-when-significant-dirs.md` (definite headline for the no-router case; the `routing_unresolved` state is that record's amended exception — see §4 D3). A **bare** `resolvedRefsFromRoots === 0` trigger would wrongly flip that repo's coverage to `null`, regressing the invariant and its test. The `null` fires **only** in the `routing_unresolved` state.
...
- **D3 — `coverage` routing-basis guard (scoped to `routing_unresolved`).** When **routers are present but none resolve** (`routing_files > 0 && resolvedRefsFromRoots === 0`), `coverage` reports `null` ("not assessed"), not a floored 0. Floor-to-0 is preserved for the **`root_absent`** case (no routers at all) → headline 0 in the worst case; and for the ≥ 1-edge-resolves-but-covers-nothing case (monolithic-route-nowhere). **Amends** `planning/decisions/2026-08-20_headline-definite-when-significant-dirs.md`: a `routing_unresolved` repo with ≥ 1 significant directory now yields `coverage`/`orphans`/`routing_drift` all null → **headline null**, narrowing that record's "≥ 1 significant dir → never null" rule to except the routers-present-but-unreadable state. That record is edited to carry the exception (one ruling, not two documents that disagree); the `root_absent` floor is untouched. `null` fires only in the `routing_unresolved` state.
...
- **D3 regression.** A no-router repo with ≥ 1 significant directory (`root_absent`, `routing_files === 0`) must keep `coverage` floor-to-0 → headline **0**; the **two existing** `headline-definite-when-significant-dirs` regression tests stay **unchanged and green** (the scoped guard must not flip them to `null`). **New positive test for the amended invariant (case 3):** a `routing_unresolved` fixture — routers present, zero resolved from any root, **WITH ≥ 1 real significant directory** — asserting **`coverage: null` AND `headline: null`**. The fixture MUST carry ≥ 1 significant dir; a 0-significant-dir fixture would dodge the conflict and prove nothing new.
