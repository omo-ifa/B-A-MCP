...
> **Forward-reference — two shape exclusions authorized, NOT yet handled** (`planning/decisions/2026-08-25_tbd-16-residual-fp-mechanisms-abc.md`). Two residual-FP shapes are authorized for a **shape exclusion** — a `...`-ellipsis segment, and a bare filename with no path segment — on measured evidence of zero false-negative risk (no real corpus path contains `...`; no resolving bare-filename route exists in the corpus). **They are still flagged today.** They become true only when their build loop lands (design → plan → TDD → review → finish), which carries its own `src/API.md` edit (rule 8). Until then they remain flagged, and re-validation must **not** treat them as handled.
