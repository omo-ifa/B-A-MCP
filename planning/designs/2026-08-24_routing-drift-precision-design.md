...
**Added 2026-08-25** (`planning/decisions/2026-08-25_tbd-16-residual-fp-mechanisms-abc.md`, the sixth ratchet trip). Re-validation run #1 surfaced four further accepted-FP classes and two shape exclusions. The four accepted classes are named here now (the tool already produces them; naming lets re-validation classify rather than bounce them). The two shape exclusions are **not yet handled** — see the forward-reference below.

- **Compound / lexical placeholder — an accepted false positive.** A path whose segment is a conventional fill-in name that mimics a real sibling family: posthog `signals-scout-foo/SKILL.md`, where `foo` stands in for a new scout and is **indistinguishable from the real `signals-scout-*` family** (30+ dirs). Only the surrounding prose ("Creating a *new* …") marks it; nothing syntactic does. A word-list shape rule would carry a false-negative risk on real names of that shape (rule 7). **Distinct from the `...`-ellipsis case below, which *is* shape-excludable.** Do not re-diagnose as a bug.
- **Path-shaped provenance citation — an accepted false positive, with a NAMED LIMITATION.** A path-shaped backtick used *referentially* rather than as a route: caveman `engine/CLAUDE.md`'s parenthetical `(bench/agent/README.md)`. Because it carries path segments, no shape rule reaches it, and the only signal — the surrounding prose — is one **the tool does not read**. **Limitation:** the tool can misread a path-shaped citation as a broken route; separating citation ("mentioning something") from route ("pointing somewhere") needs prose semantics it does not have. Accepted, named, not fixed. (A *bare-filename* reference with no path segment — posthog's nav-label `CONTEXT.md`, caveman's file-type `SKILL.md` — is a different, shape-excludable case; see the forward-reference below.)
- **Ancestor-located prose-relative under a nested router — an accepted false positive.** A nested router's prose describes a file that exists in an **ancestor** directory, outside the router's own subtree: posthog `.../features/canvas/AGENTS.md` → `docs/CANVAS-FREEFORM-REACT-PLAN.md`, real file at the ancestor `products/desktop/docs/`. Reaching it means resolving against ancestors, and ancestor-up-to-root ≈ the repo-wide search §3.1 **already rejected** ("a router excuse any path"). Exactly one legal disposition: accept the class.
- **In-subtree exact-tail-miss under a nested router — an accepted false positive.** The target exists in the router's **own** subtree but the prose prepended a spurious segment, so tier 2's exact-tail match missed it: icm `assets/templates/CLAUDE.md` → `setup/questionnaire.md`, real file `assets/templates/questionnaire.md` (no `setup/` dir). This looked narrowly fixable (a within-subtree loosening does not touch the bound), but measurement rejected the fix: the only rule that reaches it is **basename-only** (maximum-masking; a stricter suffix rule matches nothing), yield is **1 and non-recurring**, and that loosening silences **4** unrelated findings by coincidental basename collision — including a **34-way** `SKILL.md` collision — the silent-swallow direction §3.4 refuses. Accepted rather than fixed.

> **Two shape exclusions — HANDLED** (implemented 2026-08-25, PR #30; re-validated by `planning/calibration/2026-08-25_context-audit-tbd-16-revalidation-run2.md`). Two residual-FP shapes are now excluded by the routing-path shape definition (`isRoutingPathShape`), on measured zero false-negative risk: a **`...`-ellipsis segment** (no real corpus path contains a run of 3+ dots) and a **bare filename with no path segment** (no resolving bare-filename route exists in the corpus; a resolving span is already an edge by existence and never reaches the shape test). Re-validation run #2 confirmed the exclusions removed 12 findings, every one verified prose/placeholder — zero false negatives. The bare-filename exclusion also absorbed 7 of the 9 findings the root-located-router class below originally held (they were bare filenames); that class's corrected population is stated there.
