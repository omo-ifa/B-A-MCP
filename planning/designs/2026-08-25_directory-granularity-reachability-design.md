...
After the existing document-edge reachability DFS, a document is **also** reachable if it is **directly contained in a directory that was itself the target of a resolved routing edge** — i.e. a resolved edge whose target *is a directory* `D` marks every in-scope document whose parent directory is `D` as reachable.

**The propagation basis is directory *targets* only — not `routedDirs`.** `routedDirs` (which `coverage` reads) is populated three ways: a directory target adds the directory itself, but a *document* or non-doc-file target adds its **parent** directory for coverage accounting. Propagating from the whole of `routedDirs` would let a routed *document* rescue its siblings (e.g. a router pointing at `src/CONTEXT.md` would mark every doc in `src/` reachable — wrong, and it would break the existing document-granular orphan behavior). Reachability therefore propagates from a **distinct set of directory-target directories**, recorded only when a resolved edge's target is itself a directory. `routedDirs` is **not** changed, so `coverage` is untouched; the candidate set (`underRoutedDir`, a subtree test) is unchanged; `orphanCount` remains "candidates not reachable."

Nothing else moves. No new edge is created (so the routing DAG, `coverage`, `routedDirs`, and drift accounting are untouched); this is purely a widening of what counts as *reached*, sourced from directory-target edges at directory-only depth.
...
- **convention-discovered.** Files under a directory that carries a `SKILL.md`/manifest, loaded by runtime convention rather than a routing edge (superpowers' skill-support files).
