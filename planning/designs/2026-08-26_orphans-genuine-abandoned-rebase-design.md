...
**`dirTargets` — the input set:** the union, over all source docs, of the directories each routed *as a directory target*. It is **exposed from `buildGraph` as `GraphResult.dirTargets`** and threaded into `AcceptedLayoutCtx` **in place of** its `routedDirs` field (D1 is `AcceptedLayoutCtx`'s only consumer of that field, so the field is replaced, not kept alongside). `routedDirs` itself is unaffected — it stays computed inside `buildGraph` and still feeds `underRoutedDir` (the orphan-candidate gate), which is a **separate** graph-local set, not the `AcceptedLayoutCtx`. The union is **all-source, not reached-restricted** — D1 evaluates only the residual (nested) orphan set, so membership is a structural "does this repo route to this directory" fact independent of that route's own reachability.
