...
**Edge case (documented, left counted):** a doc directly inside a directory that is in `dirTargets` only because an **unreached** non-root doc routed it. Its nearest routing-known ancestor *is* the immediate parent, so D1 does **not** fire; it stays counted (genuine-abandoned). This is the safe direction — such a doc's only route is itself unreachable, which is plausibly real rot, and mis-counting it is a visible FP, not a silent FN.
