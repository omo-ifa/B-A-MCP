...
> **task-observer's `null` headline is correct — NOT a bug** (an earlier draft of this doc wrongly flagged it). task-observer has **zero significant directories** (zero source files anywhere; 6 `.md`, 2 `.yml`, 2 `.png`, 1 `.txt`, 1 `.json`). So it is the **"nothing to route"** case, not the "no context layer" worst case, and `null` ("not assessed") is honest. The genuine worst case — **no routers + ≥1 significant directory** — already scores **0** (coverage floors to 0 and anchors the headline), verified empirically. Ratified as an invariant: `planning/decisions/2026-08-20_headline-definite-when-significant-dirs.md` (headline is a definite number whenever ≥1 significant dir; `null` allowed at 0). No code change; a regression test locks it in the code pass.
