...
- **`main` HEAD:** `4d3c30e` (`fix(prompts): genericize intake/handoff gates — remove Ebo Chart domain leak (#63)`), with this refresh PR on top. Working tree **clean**. **`override_log` is built, merged, and verified on trunk** (PR #59, landed at `26c30cf`); the session's closeout then landed on top: **`/handoff`** (#60 — API.md status → Shipped, Roadmap, CLAUDE.md, this file), the **task-observer backlog review** (#61 — Obs 22–28 → WORKFLOW.md), a **handoff refresh** (#62), and the **gate-prompt genericization** (#63 — `intake.md`/`handoff.md` de-leaked; decision `planning/decisions/2026-08-27_genericize-gate-prompts-domain-leak.md`).
...
- **task-observer backlog reviewed 2026-08-27:** Obs 22–28 distilled into WORKFLOW.md's "Review-derived checklists" and marked ACTIONED (PR #61). **Obs 26** ("mirror X" must name which axes) is now applied in **both** places — the WORKFLOW.md rule **and** the source, the `/handoff` gate's next-session-starter guidance in `prompts/handoff.md` (owner reversed the earlier defer once the rule-1 cost was seen to be nil — the `.claude/commands/` generator isn't built yet). **Obs 29 logged (OPEN)** — a copied template inherits its source project's domain; audit/genericize before shipping (the lesson from the gate-prompt genericization). Backlog: **1 OPEN (Obs 29)**, for the next review.
- **Gate-prompt genericization (this session, PR #63):** `prompts/intake.md` + `prompts/handoff.md` were templated from the sibling Ebo Chart dispensary project and leaked its domain (`src/ERD.md`/schema, `consumer/dispensary/cultivator`). De-leaked — schema reads made conditional ("if the project has one"), vocabulary role-neutral; the other three gates (`problem-fit`/`decisions`/`design-doc`) were verified clean. B-A-MCP can now dogfood its own `/intake` and `/handoff` without the ERD/dispensary misfire. Decision: `planning/decisions/2026-08-27_genericize-gate-prompts-domain-leak.md`.
