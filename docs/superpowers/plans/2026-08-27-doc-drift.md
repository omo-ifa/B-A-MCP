...
> **Execution note (B):** a fourth comparison branch was folded into `diff.ts` during execution — a `shape()` helper + an `else if (dHas !== cHas)` branch so a doc object-with-properties node facing a non-opaque typed leaf emits `type_mismatch` instead of counting silently in sync (the type-list compare misses it when the object side omits an explicit `type`). Covered by the "object-vs-leaf shape mismatch" test. Ships in commit `1a683c4`.
...
Add a `### doc_drift` section after the `### override_log` section, documenting: the tool's purpose (schema-of-record drift, free/keyless/file-free); the input schema (`pairs` array of `{label?, declared, canonical}`); the output schema (`{ score, findings[], stats, rendered }`, with `findings.items` and `stats` kept as bare `{ type: "object" }` in the declared schema to hold the rule-2 budget, the full shape documented in prose); the drift categories (`field_only_in_doc` high, `field_only_in_canonical` medium, `type_mismatch` high, `required_drift` high both directions) and severity-by-drift-kind; the score definition (share of in-sync field-paths, null on empty denominator, opaque paths excluded); the entry-id note (`stableId` duplicated, discriminator = category + label + field-path); the `INVALID_PAIRS` error surface (non-array `pairs`, a non-object pair member, OR a present-but-non-object `declared`/`canonical` → error; an absent `declared`/`canonical` and an empty `pairs` array are valid, the latter → `score: null`); a `stats` note that `by_kind` may exceed `drifted` (one field-path can carry two drift kinds — e.g. `required_drift` + `type_mismatch` — so the two do not reconcile); and the invariants + accepted-limitations (opaque-wildcard deep-drift blindness with its documented recourse; colliding explicit labels → colliding ids). Mirror the depth and structure of the existing `### override_log` section.
...
- Object-vs-leaf shape drift (B, added during execution) — a doc object-node facing a typed leaf emits `type_mismatch` rather than counting silently in sync — Task 2 `diff.ts` `shape()` branch + test. ✔
...
- `INVALID_PAIRS` for a malformed call — non-array `pairs`, a non-object pair member, AND a present-but-non-object `declared`/`canonical` (`runDocDrift` boundary check); absent `declared`/`canonical` + empty `pairs` array stay valid (D5) — Task 5 (`runDocDrift` + index tests). ✔
