...
export function scoreBloat(walk: WalkResult): BloatResult {
  ...
  let routingTokens = 0;
  let penalty = 0;
  let topRouter: string | null = null;
  let topRouterTokens = 0;

  for (const r of routers) {
    const content = r.content as string;
    const tks = countTokens(content);
    routingTokens += tks;
    if (tks > topRouterTokens) {
      topRouterTokens = tks;
      topRouter = r.relPath;
    }
  ...
  if (routingTokens > TBD_11_ROUTING_TOKEN_CUTOFF) {         // TODO: TBD-11
    findings.push(low(topRouter || "CLAUDE.md", "total routing token weight is high", `routing_tokens=${routingTokens}`, "routing_token_weight"));
