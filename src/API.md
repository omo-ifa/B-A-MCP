    ...
    "findings": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "category": { "enum": ["orphan", "broken_ref", "routing_drift", "routing_path_missing", "malformed_link", "escapes_root", "coverage", "coverage_test", "bloat", "root_absent", "root_empty", "routing_unresolved", "name_collision", "symlink", "skipped"], "description": "routing_path_missing (high) is a path-shaped backtick in a ROUTER doc that resolves to nothing — a broken route, counted toward routing_drift, tallied separately from broken markdown links. broken_ref is now `info` (non-router broken links are reported, not scored). routing_unresolved (info) fires when routing files are present but none of their references resolve — a possible unrecognized routing syntax." },
