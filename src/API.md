  ...
  "properties": {
    ...
    "findings": {
      ...
      "items": {
        ...
        "properties": {
          "id": { "type": "string", "description": "findingId(\"override_field_missing\", <entry discriminator>, <field name>) — a stable sha256(...).slice(0,12) over the entry's identity discriminator and the missing field, never the moving evidence." },
...
**Entry id.** Each entry carries a stable id = `sha256(gate\0date\0decision).slice(0,12)` — a discriminator over the *identity* fields, never the moving field values, so it survives `risk` / `alternative` / `acknowledged_by` being completed. It is a reference/dedup key and the key `export_record` supersedes a prior version against — **not** a tamper-evidence guarantee: the id travels with the record and anyone holding it can recompute it. Unfakeability is the paid tier's property; this free tool does not assert it. The formula is identical to `context_audit`'s `findingId` (locked by a boundary test) so ids are comparable across tools.
